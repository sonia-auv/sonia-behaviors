...
class search_tables(EventState):
    ...
    def __init__(self, boxX=5, boxY=5, stroke=0.8 , side = False ):
        ...
        super(search_tables, self).__init__(outcomes=['continue'],
    ...
    def execute(self, userdata):
        ...
        for i in range(self.fullStep):
            ...
            # move sub foward
            new_traj.pose.append(navUtils.addpose(0, self.stroke, 0, 0, 0, 0, 1, 0, self.radius,False))

            # move sub sideway
            new_traj.pose.append(navUtils.addpose(self.get_direction()*(self.boxY), 0, 0, 0, 0, 0, 1, 0, self.radius,False))
