...
class get_bottom_vision_target(EventState):
    ...
    def on_enter(self, userdata):
        ...
        self.timeout_pub.publish(navUtils.missionTimerFunc("get_bottom_vision_target", self.param_timeout, self.uniqueID, 1))
        ...
        if self.parse_data == True:
            ...
            if self.position_reached == True and self.alignement_reached == True:
                if self.cam_bottom == True :
                    ...
                    userdata.camera = 1
                self.timeout_pub.publish(navUtils.missionTimerFunc("get_bottom_vision_target", self.param_timeout, self.uniqueID, 2))
                return 'success'
                ...
                self.timeout_pub.publish(navUtils.missionTimerFunc("get_bottom_vision_target", self.param_timeout, self.uniqueID, 4))
                return 'failed'
        if actual > self.param_timeout :
            self.timeout_pub.publish(navUtils.missionTimerFunc("get_bottom_vision_target", self.param_timeout, self.uniqueID, 3))
            return 'search'
