...
class square_movement(EventState):
    ...
    def execute(self, userdata):
        ...
        # Do a square path
        new_traj.pose.append(navUtils.addpose(self.boxX/2, 0, 0, 0, 0, 0, 1, self.speed, 0, False))
        new_traj.pose.append(navUtils.addpose(0, self.boxY/2, 0, 0, 0, 0, 1, self.speed, 0, False))
        new_traj.pose.append(navUtils.addpose(-self.boxX, 0, 0, 0, 0, 0, 1, self.speed, 0, False))
        new_traj.pose.append(navUtils.addpose(0, -self.boxY, 0, 0, 0, 0, 1, self.speed, 0, False))
        new_traj.pose.append(navUtils.addpose(self.boxX, 0, 0, 0, 0, 0, 1, self.speed, 0, False))
        new_traj.pose.append(navUtils.addpose(0, self.boxY/2, 0, 0, 0, 0, 1, self.speed, 0, False))
        new_traj.pose.append(navUtils.addpose(-self.boxX/2, 0, 0, 0, 0, 0, 1, self.speed, 0, False))

        # Turn 90
        new_traj.pose.append(navUtils.addpose(0, 0, 0, 0, 0, 90, 1, self.speed, 0, False))
