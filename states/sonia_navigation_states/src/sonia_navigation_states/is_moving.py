...
class is_moving(EventState):
    ...
    def on_enter(self, userdata):
        ...
        self.timeout_pub.publish(missionTimerFunc("is_moving", self.param_timeout, self.launch_time, 1))
    ...
    def execute(self, userdata):
        if self.is_alive:
            ...
            if self.time_diff > self.param_timeout or self.target_reached or self.stopped:
                if self.stopped:
                    self.timeout_pub.publish(missionTimerFunc("is_moving", self.param_timeout, self.launch_time, 4))
                    Logger.log('The sub is not moving, bypassing Target Reached', Logger.REPORT_HINT)
                    return 'stopped'
                elif self.target_reached:
                    self.timeout_pub.publish(missionTimerFunc("is_moving", self.param_timeout, self.launch_time, 2))
                    Logger.log('Finally got target_reached', Logger.REPORT_HINT)
                    return 'stopped'
                else:
                    self.timeout_pub.publish(missionTimerFunc("is_moving", self.param_timeout, self.launch_time, 3))
