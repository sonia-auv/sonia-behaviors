...
class ImportantFunction(EventState):
    ...
    def execute(self, userdata):
        ...
        Logger.loghint(f"Images aquired, starting calc")
        avg_top = sum([x.top for x in self.__queue]) / len(self.__queue) * self.__screen_height
        avg_left = sum([x.left for x in self.__queue]) / len(self.__queue) * self.__screen_width
        avg_bottom = sum([x.bottom for x in self.__queue]) / len(self.__queue) * self.__screen_height
        avg_right = sum([x.right for x in self.__queue]) / len(self.__queue) * self.__screen_width
