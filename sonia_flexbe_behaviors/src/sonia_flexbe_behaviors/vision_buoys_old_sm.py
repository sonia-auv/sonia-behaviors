...
@author: CS
'''
class vision_buoys_oldSM(Behavior):
	'''
	Detect the buoy and align with old align state
	'''


	def __init__(self):
		super(vision_buoys_oldSM, self).__init__()
		self.name = 'vision_buoys_old'
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('check_moving',
										...
										transitions={'stopped': 'get_buoy', 'moving': 'wait_target_reached', 'error': 'stop_filter_fail'},
			...
			# x:870 y:93
			OperatableStateMachine.add('get_buoy',
			...
			OperatableStateMachine.add('search_zigzag',
										...
										transitions={'finished': 'get_buoy', 'failed': 'stop_filter_fail', 'lost_target': 'stop_filter_lost', 'controller_error': 'stop_filter_fail'},
			...
			OperatableStateMachine.add('wait_target_reached',
										...
										transitions={'target_reached': 'get_buoy', 'target_not_reached': 'check_moving', 'error': 'stop_filter_fail'},
