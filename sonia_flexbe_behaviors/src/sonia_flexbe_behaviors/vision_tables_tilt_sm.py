...
from sonia_flexbe_behaviors.search_tilt_sm import search_tiltSM
...
@author: GS
'''
class vision_tables_tiltSM(Behavior):
	'''
	Find the tables
	'''
	...
	def __init__(self):
		super(vision_tables_tiltSM, self).__init__()
		self.name = 'vision_tables_tilt'
		...
		self.add_behavior(search_tiltSM, 'search_tilt')
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('get_bins',
										...
										transitions={'success': 'stop_success', 'align': 'align', 'move': 'align', 'failed': 'failed', 'search': 'search_tilt'},
			...
			# x:378 y:304
			OperatableStateMachine.add('search_tilt',
										self.use_behavior(search_tiltSM, 'search_tilt'),
										transitions={'finished': 'get_bins', 'failed': 'failed', 'lost_target': 'lost_target', 'controller_error': 'controller_error'},
