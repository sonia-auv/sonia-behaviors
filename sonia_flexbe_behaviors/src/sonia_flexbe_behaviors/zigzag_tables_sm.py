...
from sonia_navigation_states.search_tables import search_tables
...
@author: GS
'''
class zigzag_tablesSM(Behavior):
	'''
	zigzag table state
	'''
	...
	def __init__(self):
		super(zigzag_tablesSM, self).__init__()
		self.name = 'zigzag_tables'
	...
	def create(self):
		...
		with _state_machine:
			...
			# x:356 y:171
			OperatableStateMachine.add('zig_tables',
										search_tables(boxX=self.boxX, boxY=self.boxY, stroke=self.stroke, side=False),
			...
			OperatableStateMachine.add('init',
										...
										transitions={'continue': 'zig_tables'},
										autonomy={'continue': Autonomy.Off},
