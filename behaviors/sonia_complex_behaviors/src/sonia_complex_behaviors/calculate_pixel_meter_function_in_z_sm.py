...
@author: Nimai
'''
class CalculatepixelmeterfunctioninzSM(Behavior):
	'''
	...
	def __init__(self):
		super(CalculatepixelmeterfunctioninzSM, self).__init__()
		self.name = 'Calculate pixel meter function in z'

		# parameters of this behavior
		self.add_parameter('gate_obj_topic', '')

		# references to used behaviors
		self.add_behavior(SinglePoseMoveSM, 'Move Down')
		self.add_behavior(SinglePoseMoveSM, 'Move back to origin')
		self.add_behavior(SinglePoseMoveSM, 'Move up')
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('Get blob size origin (2)',
										...
										transitions={'success': 'Move up', 'failed': 'failed'},
										autonomy={'success': Autonomy.Off, 'failed': Autonomy.Off},
										remapping={'calc_block': 'calc_block'})

			# x:287 y:239
			OperatableStateMachine.add('Get blob size pt 1',
										get_blob_size(filterchain_obj_topic=self.gate_obj_topic, dist_from_origin=-0.3, nb_img=10, direction=0),
										transitions={'success': 'Move back to origin', 'failed': 'failed'},
										autonomy={'success': Autonomy.Off, 'failed': Autonomy.Off},
										remapping={'calc_block': 'calc_block'})
			...
			OperatableStateMachine.add('Move Down',
										self.use_behavior(SinglePoseMoveSM, 'Move Down',
											parameters={'positionX': 0.0, 'positionY': 0.0, 'positionZ': 0.3, 'orientationZ': 0.0}),
										transitions={'finished': 'Get blob size pt 1', 'failed': 'failed'},
										autonomy={'finished': Autonomy.Inherit, 'failed': Autonomy.Inherit})

			# x:512 y:248
			OperatableStateMachine.add('Move back to origin',
										self.use_behavior(SinglePoseMoveSM, 'Move back to origin',
											parameters={'positionX': 0.0, 'positionY': 0.0, 'positionZ': -0.3, 'orientationZ': 0.0}),
			...
			OperatableStateMachine.add('Move up',
										self.use_behavior(SinglePoseMoveSM, 'Move up',
											parameters={'positionX': 0.0, 'positionY': 0.0, 'positionZ': -0.3, 'orientationZ': 0.0}),
			...
			OperatableStateMachine.add('origin',
										get_blob_size(filterchain_obj_topic=self.gate_obj_topic, dist_from_origin=0.0, nb_img=10, direction=0),
										transitions={'success': 'Move Down', 'failed': 'failed'},
										autonomy={'success': Autonomy.Off, 'failed': Autonomy.Off},
										remapping={'calc_block': 'calc_block'})
